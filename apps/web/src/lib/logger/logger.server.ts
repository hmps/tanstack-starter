 ...
 *
 * Why custom Axiom transport?
 * - Official @axiomhq/pino has Vite bundling issues
 * - Full control over batching and error handling
...
function createTransport() {
  ...
  if (isProduction) {
    // Production: Axiom via custom transport
    ...
    return pino.transport({
      target: path.join(__dirname, 'axiom-transport.ts'),
      level: process.env.PINO_LOG_LEVEL || 'info',
    });
  }
